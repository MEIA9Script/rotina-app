...
const s: Record<string,React.CSSProperties> = {
  ...
  label: { display:'block', fontSize:11, fontWeight:700, letterSpacing:2, textTransform:'uppercase' as const, color:'rgba(220,232,240,0.4)', marginBottom:6 },
  ...
  const handle = async () => {
    ...
    try {
      if (isNew) {
        ...
        // Seed via API usando service role (bypassa RLS)
        if (data.user) {
    ...
    <div style={s.page}>
      <div style={s.card}>
        ...
        <button style={{...s.btn,...s.btnPrimary}} onClick={handle} disabled={loading}>
          {loading ? 'Aguarda...' : isNew ? '🚀 Criar conta' : '→ Entrar'}
        </button>
        <div style={s.toggle}>
